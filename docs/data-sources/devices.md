...
- `items` (Attributes List) Array of ResponseDevicesGetOrganizationDevices (see [below for nested schema](#nestedatt--items))
...
- `beacon_id_params` (Attributes) Beacon Id parameters with an identifier and major and minor versions (see [below for nested schema](#nestedatt--item--beacon_id_params))
...
- `major` (Number) The major number to be used in the beacon identifier
- `minor` (Number) The minor number to be used in the beacon identifier
